...
namespace Conkist.GDK.Utils
{
    ...
    public class Share
    {
    	...
    	private static AndroidJavaClass AJC
    	{
    		...
    		{
    			if( m_ajc == null )
    				m_ajc = new AndroidJavaClass( "com.yasirkula.unity.Share" );
    	...
    	private static extern void _Share_Share( string[] files, int filesCount, string subject, string text, string link );
    #endif
    	...
    	public Share SetText( string text )
    	{
    	...
    	public Share SetCallback( ShareResultCallback callback )
    	{
    		this.callback = callback;
    		return this;
    	}

    	public Share AddTarget( string androidPackageName, string androidClassName = null )
    	{
    #if UNITY_EDITOR || UNITY_ANDROID
    	...
    	public Share AddFile( string filePath, string mime = null )
    	{
    	...
    	public Share AddFile( Texture2D texture, string createdFileName = "Image.png" )
    	{
    		if( !texture )
    	...
    	public Share AddEmailRecipient( string emailAddress )
    	{
    #if UNITY_EDITOR || UNITY_ANDROID
    ...
    #elif UNITY_ANDROID
    		AJC.CallStatic( "Share", Context, new ShareResultCallbackAndroid( callback ), targetPackages.ToArray(), targetClasses.ToArray(), files.ToArray(), mimes.ToArray(), emailRecipients.ToArray(), subject, CombineURLWithText(), title );
    #elif UNITY_IOS
    		ShareResultCallbackiOS.Initialize( callback );
    		if( files.Count == 0 )
    			_Share_Share( new string[0], 0, subject, text, GetURLWithScheme() );
    		else
    		{
    			...
    			_Share_Share( files.ToArray(), files.Count, subject, CombineURLWithText(), "" );
    		}
    #else
    		Debug.LogWarning( "Share is not supported on this platform!" );
    #endif
    	}
